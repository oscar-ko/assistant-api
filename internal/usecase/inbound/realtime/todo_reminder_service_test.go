...
func TestBuildImplicitReplyTodoPromptUsesChronologicalOrderForReplySemantics(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"先判 Current message 的語用類型，再判是否承接既有 Todo candidate context，最後才判欄位完整性",
		"distinctive task phrase",
		"不可只因為兩個 candidate 都有整理、清單、回報、Oscar、期限等泛用元素",
		"必須輸出 update_candidate 或 acknowledge",
		"不可輸出 create_candidate",
		...
		"若它只是另一個話題的狀態通知或閒聊",
		"即使 Context messages 裡有 active candidate，也必須 no_action",
...
func TestBuildImplicitReplyTodoPromptMarksLatestPriorMessageAsAdjacencyTarget(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"decision precedence",
		"Todo candidate contexts 中同一任務的補充、限制、負責人、期限或交付格式",
		"Current message 本身與既有 context 明顯不同的新任務內容或替代安排",
		"Latest prior conversation message 的短確認/短否定/可行性答覆",
		"若前面任一層成立，不可再因 Current message 本身很短或缺欄位而改成 no_action",
		"Current message 本身包含任務內容/交付/時間安排時，若無法對應任何 Todo candidate context",
		`"decision":"create_candidate"`,
		"Current message 本身提出新的可追蹤交付內容與時間安排，且不同於既有 candidate context",
