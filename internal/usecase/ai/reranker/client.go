...
func (c *client) probeAliveOnce(ctx context.Context) error {
	...
	if err != nil {
		probeErr := fmt.Errorf("invalid reranker base url: %w", err)
		becameUnhealthy, _ := c.storeProbeResult(probeErr)
		if becameUnhealthy && shouldLogProbeTransition() {
			zap.L().Error("reranker alive probe failed", zap.String("base_url", c.baseURL), zap.Error(probeErr))
		}
		return probeErr
	}
	host := strings.TrimSpace(parsed.Host)
	if host == "" {
		probeErr := fmt.Errorf("reranker base url has empty host")
		becameUnhealthy, _ := c.storeProbeResult(probeErr)
		if becameUnhealthy && shouldLogProbeTransition() {
			zap.L().Error("reranker alive probe failed", zap.String("base_url", c.baseURL), zap.Error(probeErr))
		}
	...
	if dialErr != nil {
		...
		if becameUnhealthy && shouldLogProbeTransition() {
			zap.L().Error("reranker alive probe failed", zap.String("base_url", c.baseURL), zap.String("host", host), zap.Error(dialErr))
		}
		return dialErr
	}
	_ = conn.Close()
	...
	if recovered && shouldLogProbeTransition() {
		zap.L().Info("reranker alive probe recovered", zap.String("base_url", c.baseURL), zap.String("host", host))
	}
	return nil
}
