...
func (_q *SkillQuery) QueryActions() *ActionQuery {
...
func (_q *SkillQuery) WithActions(opts ...func(*ActionQuery)) *SkillQuery {
	query := (&ActionClient{config: _q.config}).Query()
...
func (_q *SkillQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Skill, error) {
	...
	if query := _q.withActions; query != nil {
		if err := _q.loadActions(ctx, query, nodes,
			func(n *Skill) { n.Edges.Actions = []*Action{} },
			func(n *Skill, e *Action) { n.Edges.Actions = append(n.Edges.Actions, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *SkillQuery) loadActions(ctx context.Context, query *ActionQuery, nodes []*Skill, init func(*Skill), assign func(*Skill, *Action)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(action.FieldSkillID)
	}
...
func (_q *SkillQuery) WithNamedActions(name string, opts ...func(*ActionQuery)) *SkillQuery {
	query := (&ActionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
